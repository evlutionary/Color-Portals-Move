...
public class EntityListener implements Listener {
    ...
    public void onSignWrite(SignChangeEvent event) {
		...
		String move = event.getLine(2);
                if (move.length() == 0) {
                    event.getPlayer().sendMessage(ChatColor.RED + "This portal cannot be moved");
                    return;
                }
		else {
		    event.getPlayer().sendMessage(ChatColor.GREEN + "This portal can be moved.");
                    return;
                }
                ...
                if (plugin.getPortalListener().portalCanBeCreated(channel, color)) {
                    ...
                    Portal portal = new Portal(event.getPlayer().getUniqueId(), name, color, channel, node, event.getBlock().getLocation(), move);
    ...
    public void onPlayerButtonPress(PlayerInteractEvent event) {
        ...
        if (portal == null) && (plugin.getUsePerms() && !(event.getPlayer().hasPermission("colorportals.move.*") || event.getPlayer().hasPermission("colorportals.move."+colorString))) {
            player.sendMessage(ChatColor.DARK_RED + "You are not authorized to move "+colorString+" portals or this portal pair does not exist");
        }
	else {
	    MovePortalEvent e = new MovePortalEvent(portal, event.getPlayer());
                Bukkit.getServer().getPluginManager().callEvent(e);
	}
